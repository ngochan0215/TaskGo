  ...
  try {
    ...
    const discounts = await Discount.find({
      ...
      $and: [
        ...
        // Theo task/service cụ thể (nếu có task_ids thì phải match, nếu không có thì áp dụng toàn bộ)
        {
          $or: [
    ...
    // Lấy discount có priority cao nhất (đã sort ở trên)
    const bestDiscount = discounts[0];

    // Kiểm tra min_order_value nếu có
    if (bestDiscount.conditions?.min_order_value && basePrice < bestDiscount.conditions.min_order_value) {
      return {
        discount: null,
        discountAmount: 0,
        finalPrice: basePrice,
