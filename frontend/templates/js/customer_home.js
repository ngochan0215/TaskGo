...
const TASK_DETAIL_MAP = {
  cleaning_washing_machine: "./detail_pages/cleaning_washing_machine.html",
  cleaning_house: "./detail_pages/cleaning_house.html",
  cleaning_air_conditioner: "./detail_pages/cleaning_air_conditioner.html",
...
const PROMO_COLORS = [
  {
    bg: "#5c956c",
    badge: "bg-primary-100 text-primary-500 border-primary-200",
  },
...
async function loadPromotions() {
  try {
    ...
    data.promotions.forEach((promo, index) => {
      ...
      card.className =
        "flex-shrink-0 w-[280px] bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden cursor-pointer hover:shadow-md transition-shadow";
      card.onclick = () => openPromoModal(promo.id, promo.type);

      const typeLabel = promo.type === "discount" ? "Khuyến mãi" : "Voucher";
      const shortDesc = promo.description
...
async function openPromoModal(id, type) {
  try {
    ...
    document.getElementById("promo-modal-description").textContent =
      promo.description || "Không có mô tả";
    document.getElementById("promo-modal-discount").textContent =
      promo.discount_text;
    document.getElementById("promo-modal-dates").innerHTML = formatDateRange(
      promo.begin_date,
      promo.end_date,
    );
    ...
    if (promo.type === "discount") {
      ...
      typeBadge.className =
        "text-xs font-bold px-3 py-1 rounded-full bg-primary-100 text-primary-500 border border-primary-200";
    } else {
      typeBadge.textContent = "VOUCHER";
      typeBadge.className =
        "text-xs font-bold px-3 py-1 rounded-full bg-purple-100 text-purple-500 border border-purple-200";
    }

    // Set conditions
    const conditionsList = document.getElementById(
      "promo-modal-conditions-list",
    );
    ...
    if (conditions.days_of_week && conditions.days_of_week.length > 0) {
      const dayNames = conditions.days_of_week
        .map((d) => getDayName(d))
        .join(", ");
      conditionsList.innerHTML += `
              <div class="flex items-start gap-2">
    ...
    if (conditions.customer_tiers && conditions.customer_tiers.length > 0) {
      const tierNames = conditions.customer_tiers
        .map((t) => getTierName(t))
        .join(", ");
      conditionsList.innerHTML += `
              <div class="flex items-start gap-2">
    ...
    if (conditionsList.innerHTML === "") {
      conditionsList.innerHTML =
        '<span class="text-gray-400 text-xs">Không có điều kiện đặc biệt</span>';
    }

    // Set eligibility
    const eligibilityStatus = document.getElementById(
      "promo-modal-eligibility-status",
    );
    const eligibilityReasons = document.getElementById(
      "promo-modal-eligibility-reasons",
    );
    ...
    if (promo.is_eligible) {
      ...
      if (promo.eligibility_reasons && promo.eligibility_reasons.length > 0) {
        eligibilityReasons.innerHTML = promo.eligibility_reasons
          .map((r) => `<div class="text-red-500">• ${r}</div>`)
          .join("");
      }
    }
...
async function openTaskerModal(tasker) {
  ...
  document.getElementById("modal-tasker-rating").textContent = tasker.rating;
