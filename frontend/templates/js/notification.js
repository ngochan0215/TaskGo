const API_BASE = typeof CONFIG !== 'undefined' ? CONFIG.API_BASE_URL : "http://localhost:3000";
...
function mapIcon(type) {
    ...
    if (type === "discount" || type === "promo") return "local_offer";
...
function render() {
    ...
    const filtered = notifications.filter(n => {
        ...
        // Map filter frontend (promo) với data backend (discount) nếu cần
    ...
    filtered.forEach((item) => {
        ...
        const cardClass = isUnread
            ? 'bg-white shadow-[0_2px_8px_-2px_rgba(0,0,0,0.1)] border-l-[6px] border-green-500'
            : 'bg-white border border-slate-100 opacity-80 hover:opacity-100 hover:shadow-sm';

        const iconStyle = isUnread ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-400';
            ...
            <div class="group relative flex gap-4 p-4 rounded-lg fade-in ${cardClass}">
                <div onclick="readItem('${item.id}')" class="flex-1 flex gap-4 cursor-pointer">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs ${isUnread ? 'text-green-600 font-medium' : 'text-slate-400'} mt-1">${item.time}</p>
                    </div>
                </div>
...
function filterNotifs(type) {
    ...
    tabs.forEach(t => {
        ...
        if (t === type) {
            btn.className = "w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-green-50 text-green-700 font-bold transition-all border border-green-100 shadow-sm";
        } else {
...
async function readItem(id) {
    ...
    try {
        ...
        if (item.reference?.kind === "Order" && item.reference.refId) {
            window.location.href = `/orders/${item.reference.refId}`;
        }
    } catch (e) {
        console.error(e);
    }
...
function showToast(item) {
    ...
    toast.className = `bg-white border-l-4 border-green-500 shadow-lg rounded-lg p-4 w-80 animate-slide-in transition-all duration-300`;
    toast.innerHTML = `
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof io !== 'undefined') {
        ...
        const role = JSON.parse(localStorage.getItem("system_role"));
