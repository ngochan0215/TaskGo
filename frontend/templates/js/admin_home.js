(function () {
  ...
  const TOP_SERVICE_COLORS = ["bg-primary-500", "bg-blue-500", "bg-orange-500", "bg-purple-500", "bg-pink-500"];
  ...
  function renderCards(d) {
    ...
    const cards = [
      { value: d.totalRevenue, change: d.revenueChangePercent, valueEl: "card-revenue", changeEl: "card-revenue-change", format: function (v) { return formatCurrency(v); } },
      { value: d.newOrders, change: d.newOrdersChangePercent, valueEl: "card-orders", changeEl: "card-orders-change", format: function (v) { return String(v); } },
      { value: d.totalCustomers, change: d.customersChangePercent, valueEl: "card-customers", changeEl: "card-customers-change", format: function (v) { return String(v); } },
      { value: d.activeTaskers, change: d.activeTaskersChangePercent, valueEl: "card-taskers", changeEl: "card-taskers-change", format: function (v) { return String(v); } },
    ];
    cards.forEach(function (c) {
      ...
      if (ce) {
        ...
        ce.className = "text-xs font-bold px-2 py-0.5 rounded-full " + changeBadgeClass(c.change);
      }
    });
  }
  ...
  function renderChart(revenueByWeek) {
    ...
    const amounts = (revenueByWeek || []).map(function (x) { return x.amount || 0; });
    const max = Math.max.apply(null, amounts.concat(1));
    const labels = (revenueByWeek || []).map(function (x) { return x.label; });

    wrap.innerHTML = (revenueByWeek || []).map(function (r, i) {
    ...
    if (labelWrap) {
      labelWrap.innerHTML = labels.map(function (l) { return "<span>" + l + "</span>"; }).join("");
    }
  }
  ...
  function renderTopServices(topServices) {
    ...
    if (!topServices || topServices.length === 0) {
      container.innerHTML = '<div class="text-sm text-gray-400 text-center py-4">Chưa có dữ liệu</div>';
      return;
    }

    container.innerHTML = topServices.map(function (s, i) {
      var color = TOP_SERVICE_COLORS[i % TOP_SERVICE_COLORS.length];
      return (
        '<div><div class="flex justify-between text-sm mb-1">' +
        '<span class="text-gray-700 font-medium">' + (s.name || "N/A") + "</span>" +
        '<span class="font-bold">' + (s.percent || 0) + "%</span></div>" +
        '<div class="w-full bg-gray-200 h-2 rounded-full overflow-hidden">' +
        '<div class="' + color + ' h-full" style="width:' + (s.percent || 0) + '%"></div></div></div>'
      );
    }).join("");
  }
  ...
  function renderLatestOrders(latestOrders) {
    ...
    if (!latestOrders || latestOrders.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" class="px-4 py-6 text-center text-gray-400">Chưa có đơn hàng</td></tr>';
      return;
    }

    tbody.innerHTML = latestOrders.map(function (o) {
      return (
        "<tr class=\"hover:bg-gray-50\">" +
        '<td class="px-4 py-3 font-medium">' + (o.idDisplay || o.id) + "</td>" +
        '<td class="px-4 py-3">' + (o.customerName || "N/A") + "</td>" +
        '<td class="px-4 py-3">' + (o.serviceName || "N/A") + "</td>" +
        '<td class="px-4 py-3"><span class="px-2 py-0.5 rounded text-xs font-bold ' + statusBadgeClass(o.statusKey) + '">' + (o.status || "N/A") + "</span></td>" +
        '<td class="px-4 py-3 text-right font-bold">' + formatVND(o.amount) + "</td></tr>"
      );
    }).join("");
  }

  function loadDashboard(period) {
    var p = period || (document.getElementById("period-select") && document.getElementById("period-select").value) || "month";
  ...
  function exportReport() {
    fetch(API_BASE + "/export-pdf", { headers: { Authorization: "Bearer " + token } })
      .then(function (r) {
        if (r.status === 401) {
          localStorage.removeItem("token");
          window.location.href = "../auth/login-signup.html";
